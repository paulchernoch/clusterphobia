...
use crate::data::{load_s1, load_s2, answer_key_2d};
use crate::data::clustered_2d::Clustered2D;
use clusterphobia::clustering::bcubed::BCubed;


/// Verify that we can read the CSV file for data set S1 and parse it into Clustered2D values. 
#[test]
fn parse_s1() {
    parse_2d_test(load_s1(), 15);
}

/// Verify that we can read the CSV file  for data set S2 and parse it into Clustered2D values. 
#[test]
fn parse_s2() {
    parse_2d_test(load_s2(), 15);
}

fn parse_2d_test(csv_data : &str, expected_categories : i32) {
    ...
    for (expected_id, record) in iter.enumerate() {
        match record {
            Ok(point2d) => {
                ...
                let good_category = point2d.category >=1 && point2d.category <= expected_categories;
...
/// Show that we can load a test data set (s1) with perfect categories into a Clustering, then compare 
/// it to another perfect Clustering and one with an error and get good results. 
#[test]
fn s1_answer_key() {
    let mut solution = answer_key_2d(load_s1());
    asserting("Expecting 15 clusters for s1").that(&solution.cluster_count()).is_equal_to(15);

    // If the clusterings are identical, the similarity should be one. 
    let gold = answer_key_2d(load_s1());
